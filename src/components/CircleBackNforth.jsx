  ...
  useEffect(() => {
    ...
    socketRef.current.onmessage = (event) => {
      ...
      if (Array.isArray(data)) {
        ...
        setServerMessage((prevserverMessage) => [
          ...data.reverse(),
          ...prevserverMessage,
        ]);
      } else {
  ...
  const handleSend = () => {
    ...
    if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
      socketRef.current.send(
        JSON.stringify({ sender: AuthUser.username, message: message })
      );
      setMessage("");
    }
  };
    ...
    return (
      <div key={index} className="container mb-3">
        <div className="row">
          ...
          <p className="mb-0">
            <span className="fw-bold fw-bold">
              {message.sender?.users?.legalName}
            </span>
            <span className="text-muted small">
              {" "}
              &nbsp;&nbsp;{date_format(message.date)}&nbsp; [{" "}
              {date_format(message.date, true)} ]{" "}
            </span>
          </p>
        </div>
        <div className="row">
    ...
    <div className="container my-5">
      ...
      <div className="card mx-auto ">
        <div className="card-header border-0 shadow-sm text-center">
          <h3>
            Back & Forth <br /> Circle No. {circleInfo?.code} <br />{" "}
            {circleInfo?.district?.code}{" "}
          </h3>
        </div>
        <div
          className="card-body mh-100 p-0"
