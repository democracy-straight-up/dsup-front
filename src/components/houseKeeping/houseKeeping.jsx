...
function HouseKeeping(){
    ...
    useEffect(()=>{
        if(candidate){
            candidate.map((candidate)=>{if(candidate.user.username === AuthUser.username){ setIs_candidate(true); } })
        }
    },[candidate,members])
    ...
    const handleModelShow = (param,member) =>{
        switch(param){
            ...
            case 'voteIn':
                ...
                setModelContent (`Checking this box will make ${candidate[0]?.user.users.legalName} a member of this Circle.`)
                setAct('voteIn')
                setShowModel(true)
                break
            case 'voteOut':
            ...
            if(Is_delegate){
                ...
                if(circleMembers?.length === 1){
                    return ( <> <span>
                            ...
                            onChange={()=>handleModelShow('dessolveCircle', '')} checked={showModel}/>
                        </span> </>  )
                }else{
    ...
    return (
        <div className="container">
                    ...
                    <tbody>
                        {members?.length > 0 ?
                         members?.map((member, index)=>(
                            <tr key={index} className="border">
                                ...
                                {circleInfo?.is_active ? <>
                                    <td>{Is_delegate? circleInfo?.is_active? member.voteOuts.length===0? "":member.voteOuts.length :null :null}</td>
                        ...
                        {candidate?.length > 0 ?
                            <tr>
                                ...
                                {circleInfo?.is_active ? <><td></td> <td></td></> : ""}
            ...
            <div className="row border p-3 shadow-sm">
                <p><strong>Status: </strong>{circleInfo?.is_active? "This Circle is active!"
