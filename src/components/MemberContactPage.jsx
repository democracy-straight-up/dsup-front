...
import {addCirclemMembers, authenticate} from '../store/UserSlice';
...
function MemberContactPage() {
    ...
    useEffect(() => {
        if (AuthUser && AuthUser.token && AuthUser.token.access) {
            let header = { 'Authorization': `Bearer ${AuthUser.token.access}` }
            const memberUrl = `${window.location.protocol}//${baseURL}/api/circlemember/`;
            const member_params = { group: circleInfo?.code};
            axios.post(memberUrl, member_params, { headers: header })
                .then((response) => {
                    console.log(response);
                    dispatch(addCirclemMembers(response.data));
                })
                .catch(error => {
                    console.log(error);
                });
