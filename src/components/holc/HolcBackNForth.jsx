...
const HolcBackNForth = () => {
  ...
  const holc = useSelector((state) => state.AuthUser.holc);
  ...
  useEffect(() => {
    if (!holc?.code || !AuthUser?.token?.access) {
      setError("HoLC information or authentication token not available");
      setLoading(false);
      return;
    }

    const wsProtocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    const wsUrl = `${wsProtocol}//${baseURL}/ws/holc/backnforth/${holc.code}/?token=${AuthUser.token.access}`;

    console.log("Connecting to WebSocket:", wsUrl);
    const ws = new WebSocket(wsUrl);

    ws.onopen = () => {
      console.log("HoLC BackNForth WebSocket connected");
    ...
    ws.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case "user_status":
          console.log("User status:", data.message);
          break;
    ...
    ws.onerror = (error) => {
      console.error("HoLC BackNForth WebSocket error:", error);
      setConnectionStatus("Connection Error");
      setError("WebSocket connection failed");
    };

    ws.onclose = (event) => {
      console.log("HoLC BackNForth WebSocket disconnected:", event.code, event.reason);
  ...
  }, [holc?.code, AuthUser?.token?.access]);
    ...
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `${window.location.protocol}//${baseURL}/api/holc/holc/${holc.code}/backnforth/messages/`,
          {
            headers: {
        ...
        if (error.response?.status === 403) {
          setError("You must be a HoLC member to view messages");
        } else if (error.response?.status === 404) {
          setError("HoLC not found");
        } else {
  ...
  const deleteMessage = (messageId) => {
    ...
    console.log("Deleting message:", deleteData);
