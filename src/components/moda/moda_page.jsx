...
function ModaPage() {
  ...
  const second_link = useSelector((state) => state.AuthUser.moda);
  ...
  useEffect(() => {
    ...
    const url = `${ws_scheme}://${baseUrl}/moda/${code}/${username}`;
  ...
  const action_lists = (msg) => {
    ...
    if (msg.status === "success") {
      if (msg.member_list) {
        ...
        dispatch(moda(msg.member_list[0]?.moda));

        // on each member change, check if the Circle has one member.
        if (msg.member_list.length <= 1) {
          setDissolve(true);
          console.log("eligible to Dissolve the second link...");
        } else {
          setDissolve(false);
        }
  ...
  const invitationKey = () => {
    ...
    if (socketRef.current.readyState === WebSocket.OPEN) {
      ...
      const linkCode = second_link?.code;
      ...
      <div className="row">
        ...
        <div className="col-sm-12 col-md-6 mt-3">
          ...
          <h3 className="text-center">
            Second Link No: {second_link?.code} &nbsp;&nbsp; District:
            {second_link?.district.code}
          </h3>
          <h4 className="text-center">Invitation Key: {second_link?.invitation_key}</h4>
