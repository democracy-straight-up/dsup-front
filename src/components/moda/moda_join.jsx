...
import { authenticate, moda } from "../../store/userSlice.js";
...
function JoinModa() {
  ...
  const [s_link, setSLink] = useState({});
  ...
  useEffect(() => {
    ...
    if (s_link?.code > 1) {
      ...
      const url = `${ws_schame}://${process.env.REACT_APP_BASE_URL}/moda/${s_link.code}/${AuthUser.username}/`;
  ...
  const handleJoin = () => {
    ...
    if (AuthUser?.token.access.length > 0 && inviteKey?.length === 10) {
      ...
      const url = `${window.location.protocol}//${baseURL}/api/moda/moda-members/join_invite_key/`;
      ...
      axios
        ...
        .then((response) => {
          if (response.status === 400) {
            ...
            dispatch(moda(response.data[0].moda));
            setSLink(response.data[0].moda);
    ...
    <div className="container">
      <div className="row">
        ...
        <div className="col-sm-12 col-md-6 mt-3">
          <h1 className="text-center">Join a S-Link</h1>
          <div className="">
            <h4 className="text-left">You must have an Invitation Key to join a S-Link.</h4>
            <p className="text-left">
              The Second Delegate of a S-Link should send you an Invitation Key. This cannot be done
              via the website.
            </p>
            <p className="text-left">
              Or you can create your own S-Link and start inviting others to join it.
            </p>
