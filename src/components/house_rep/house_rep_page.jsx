...
import { house_rep } from "../../store/userSlice";
import Status from "./status_message";

function House_repPage() {
  const AuthUser = useSelector((state) => state.AuthUser.user);
  const districtcouncil = useSelector((state) => state.AuthUser.house_rep);
  ...
  useEffect(() => {
    ...
    if (!districtcouncil.code || !username || !baseUrl || isConnected || isConnecting) {
      // Optional: set an error if connection can't be attempted due to missing info
      if (!districtcouncil.code || !username || !baseUrl) {
        setErr("Missing connection details.");
      }
      return;
    }

    const url = `${ws_scheme}://${baseUrl}/district-council/${districtcouncil?.code}/${username}`;
    ...
    chatSocket.onclose = (event) => {
      ...
      if (!districtcouncil.code || !username || !baseUrl || event.wasClean) {
        setErr("Disconnected. Connection closed cleanly.");
        return;
      }

      setErr("Disconnected. Attempting to reconnect...");
  ...
  }, [districtcouncil.code, username, baseUrl, ws_scheme, isConnecting]); // isConnecting is added to trigger reconnects
  ...
  const action_lists = (msg) => {
    ...
    if (msg.status === "success") {
      if (msg.member_list) {
        ...
        dispatch(house_rep(msg.member_list[0]?.district_council));

        // on each member change, check if the Circle has one member.
        if (msg.member_list.length <= 1) {
          setDissolve(true);
          console.log("eligible to Dissolve the District Council...");
        } else {
          setDissolve(false);
        }
    ...
    if (msg.action === "invitationKey") {
      ...
      dispatch(house_rep(msg.district_cauncil));
    }
  };
  ...
  const invitationKey = () => {
    ...
    if (socketRef.current.readyState === WebSocket.OPEN) {
      ...
      const house_repCode = districtcouncil?.code;
      ...
      if (house_repCode) {
        try {
          ...
          socketRef.current.send(
            JSON.stringify({
              ...
              payload: { districtcouncil: house_repCode },
            })
          );
        ...
        console.error("Cannot send invitation: Missing house_rep.code.");
            ...
            <tr>
              ...
              {Iam_delegate || Iam_member ? (
                <th className="fw-bold">
                  {dissolve === true ? "Dissolve District Council? " : "Remove Member?"}
                </th>
              ) : (
                <th></th>
              )}
